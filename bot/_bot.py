...
from api.static import START, INFO, NEW_PLASTIC, ADDRESS, CLASS_TRASH_1, CLASS_TRASH__ALL, NOT_CLASS, NOT_LOC, TRY_AGAIN, STICKER, LOC_AWARNES, LOW_BUTTON, CLASS_TYPE
...
inline_btn_1 = InlineKeyboardButton('Да', callback_data='24YBXZ')
inline_btn_2 = InlineKeyboardButton('Нет', callback_data='23NGW8')
inline_kb1 = InlineKeyboardMarkup().add(inline_btn_1).add(inline_btn_2)

inline_plastic = InlineKeyboardButton('Пластик', callback_data='P91CF2')
inline_paper = InlineKeyboardButton('Бумага', callback_data='P11RK3')
inline_metal = InlineKeyboardButton('Металл', callback_data='M72LN0')
inline_oganic = InlineKeyboardButton('Органические материалы', callback_data='O39SH1')
inline_glass = InlineKeyboardButton('Стекло', callback_data='G94KH8')
inline_composite = InlineKeyboardButton('Композитные материалы', callback_data='C02EG5')
inline_all = InlineKeyboardButton('Все сразу', callback_data='A69DJ9')
...
@dp.callback_query_handler(lambda c: c.data in ['24YBXZ', '23NGW8'], state='*')
async def process_callback_button1(callback_query: types.CallbackQuery, state: FSMContext):
    user_id = callback_query.from_user.id

    if callback_query.data == "24YBXZ":
    ...
    if len(set(class_list)) > 1 and callback_query.data != '23NGW8':
...
@dp.callback_query_handler(lambda c: c.data in ['P91CF2', 'P11RK3', 'M72LN0', 'O39SH1', 'G94KH8', 'C02EG5', 'A69DJ9'], state='*')
async def process_callback_button2(callback_query: types.CallbackQuery, state: FSMContext):
    user_id = callback_query.from_user.id
    async with state.proxy() as data:
        user_calss = data['my_classes']
    result_type = {'P91CF2':'1-9 Plastic', 'P11RK3':'20-23 Paper', 'M72LN0':'40-41 Metal', 'O39SH1':'50-61 Organic materialc', 'G94KH8':'70-79 Glass', 'C02EG5':'80-97 Composite materials', 'A69DJ9':set(user_calss)}
